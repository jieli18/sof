...
class DynamicsModel(DynamicsConfig):
    ...
    def init_policy(self):
        if self.random_init_policy:
            return torch.rand((1, 1), dtype=torch.float64) * (8/3 + 7/4) - 7/4
        else:
    ...
    def policy_noise(self):
        ...
        theta = torch.rand(1, )
        U = torch.zeros_like(self.opt_policy, dtype=torch.float64)
...
class Circuit(DynamicsConfig):
    ...
    @staticmethod
    def init_policy():
        return torch.tensor([[0.000, -1.000],
